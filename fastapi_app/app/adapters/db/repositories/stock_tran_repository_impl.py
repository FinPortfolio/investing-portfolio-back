...
from app.adapters.exceptions import StockTranNotFoundError
...
class SQLAStockTranRepository(StockTranRepository):
    ...
    async def _get_db_stock_tran_or_500(self, transaction_id: int) -> StockTranModel:
    ...
    async def get_stock_tran_by_id(self, transaction_id: int) -> StockTranEntity:
        db_stock_tran = await self._get_db_stock_tran_or_500(transaction_id)
        return db_stock_tran.to_entity()
